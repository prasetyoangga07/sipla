        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex p-2 border-bottom mb-2">
                <img src="./images/user/user.png" alt="user" class="img-user me-2">
                    <div>
                        <p style="font-size:18px ;"><?php echo $user_data['username']; ?></p>
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Data Handphone</li>
                    </ol>
                </nav>
            </div>
        </div>
...
<section class="section">
        ...
        $stmt = $conn->prepare("SELECT handphone.*, pengguna.username, status_perbaikan.status 
        FROM handphone 
        INNER JOIN status_perbaikan ON handphone.id_status = status_perbaikan.id_status 
        INNER JOIN pengguna ON handphone.id_pengguna = pengguna.id_pengguna 
        WHERE handphone.id_pengguna = ?");

        $stmt->bind_param("i", $user_data['id_pengguna']);
        ...
        while($row = $result->fetch_assoc()){
            if($row["id_status"]==2){
                ...
                <tr>
                    ...
                    <td>$row[status]</td>
                    <td><a href='checkout.php?id=$row[id_hp]'>Bayar</a></td>
                </tr>
                ";
            } else {
                echo "
