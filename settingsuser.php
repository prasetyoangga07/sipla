...
$sql = "select * from pengguna where email = '$_SESSION[email]' and password = '$_SESSION[password]'";
...
if(isset($_POST['up_prof'])){
    ...
    $sql = "select * from pengguna where email = '$email' and password = '$pass_lama'";
    $data_user = mysqli_fetch_array(mysqli_query($conn, $sql));

    if ($data_user['email'] == $email && $data_user['password'] == $pass_lama){
        if ($pass_baru == $konf_pass){
            mysqli_query($conn, "update pengguna set password = '$pass_baru' where id_pengguna = '$data_user[id_pengguna]'");
            $_SESSION['password'] = $pass_baru;
            ?> <script>alert('Password berhasil diubah')</script> <?php
        } else {
            ?> <script>alert('Konfirmasi password tidak sesuai')</script> <?php
        }
    } else {
        ?> <script>alert('Email atau password lama yang dimasukkan salah!')</script> <?php
    }
}
?>
...
<div class="card">
                <div class="card-header card-header-primary">
                  ...
                  <p class="card-category">Isi data</p>
                </div>
                <div class="card-body">
                    ...
                    <div class="row">
                      <div class="col-md-5">
                        <div class="form-group bmd-form-group">
                          <label class="bmd-label-floating"><label class="bmd-label-floating">Masukkan email</label></label>
                     ...
                     <div class="col-md-4">
                        <div class="form-group bmd-form-group">
                          ...
                          <input type="password" class="form-control" name="pass_lama" id="npwd" required>
                        </div>
                      </div>
                      ...
                      <div class="col-md-4">
                        <div class="form-group bmd-form-group">
                          ...
                          <input type="password" class="form-control" name="konf_pass" required>
                        </div>
                      </div>
               
                    <button class="btn btn-primary pull-right" type="submit" name="up_prof">Update Profil</button>
            ...
            <footer>
                <div class="footer clearfix mb-0 text-muted">
                    ...
                    <div class="float-end">
                        <p>Crafted<span class="text-danger"></span> by <a
